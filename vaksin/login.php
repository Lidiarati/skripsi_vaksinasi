...
<?php
	...
	if($_POST){
		...
		$query = mysqli_query($koneksi, "select * from admin1 where email = '".$email."' and password = '".md5($password)."'");

		if(mysqli_num_rows($query)>0){
			$_SESSION['status_admin']=true;
			echo "<script>location.href='./admin/dashboard.php'</script>";
		}else{
			$query2 = mysqli_query($koneksi, "select * from akun where email = '".$email."' and password = '".md5($password)."'");						
